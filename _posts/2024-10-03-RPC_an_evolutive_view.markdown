---
title:  "An Evolutive View Of RPC"
...
_How many times did we (as an industry) solve RPC before?_

followed by

_I wonder what was the killer feature that deprecates each old approach in favor of the new thing_
...
> Process 1: I'll put this here for Process 2<br/>
> Process 1: Hey P2 check this out<br/>
> Process 2: Oh cool<br/>
> Process 2: Got it, thanks

For this analysis, I'll define RPC as:
_"An API that allows to execution of a specific subroutine from a previously defined set of available subroutines on a remote machine."_
{: .notice--success}
...
> Process 1: (throws a rock with a paper wrapped around to Process2)<br/>
> Process 2: Hau! (look back, unwrap the paper, it says "How old is your mother?")<br/>
> Process 2: (writes "76, why?" in the same paper)<br/>
> Process 2: (throws the rock back)<br/>
> ...etc

_**rpc-v0.2.0:** Make it work for all the known tools (mv, cp, tail, etc)_
...
_**rpc-v0.4.0:** Make it work for any tool (that can be expressed in Mesa pl)_
...
> millionaire 1: hey I saw you are making money with that e-commerce thing, can I sell my stuff there?<br/>
> millionaire 2: sure! can you send me your stock articles, some photos, prices<br/>
> millionaire 1: ah... yeah... that might take me ages... Can a computer do that for me?<br/>
> millionaire 3: off course!<br/>
> millionaire 1: ?!<br/>
> millionaire 2: (who is this nerd?)<br/>
> millionaire 3: but first, let's dive into the intricacies of time sync over an infinite network of computers shall we?<br/>
> etc...
...
_**rpc-v0.5.0:** Make it work for any tool on any OS for the foreseeable future_

**About the dates:** things start to go fast from this point in time forward, I picked some technologies and dates to flatten events in a line but they are fairly overlapped.
{: .notice--info}
...
_**rpc-v0.5.1:** Make just RPC work for any tool on any OS using any development platform for the foreseeable future and become rich in the process_
...
_**rpc-v0.5.2:** Make it work over HTTP (rolling back all previous efforts)_
...
_**rpc-v0.5.3:** Make it work over HTTP in all web browsers and development platforms_
...
_**rpc-v0.6.0:** Make it less of a hack and still work over HTTP and keep compatibility_
...
_**rpc-v0.6.1:** Make it so we can evolve APIs with some control but not as strict as WSDL, and probably build some tooling around it._
...
Not much was done on the RPC side, this was just a layer over the well-established _API over HTTP_ pattern. It is worth mentioning that it also introduced streaming updates to a specific part of the dataset to the clients using WebSockets, which was _the cool thing_ at this point.

_**rpc-v0.6.2:** Tailor the RPC problem to data fetching and manipulation._
...
_**rpc-v1.0.0:** Add all the old features again: code generation for client and server, works on any platform, declarative API definitions, etc But! Don't work in the browser._

### 2024 - Not much has changed
As of today, there are two big solution spaces: client-facing ones dealing mostly with the web browser, and services specific when processes are running in less restrictive environments.

In the first group we have things like [NextJs Server Actions](https://www.reddit.com/r/nextjs/comments/1ad3q3h/comment/kjz1nju/?utm_source=share&utm_medium=web3x&utm_name=web3xcss&utm_term=1&utm_content=share_button), [Blazor with SignalR](https://stackoverflow.com/q/58212051/418543) or [Phoenix LiveView]({% post_url 2024-03-05-phoenix %}#liveview) which are in-framework solutions primarily target the web browser. They don't offer any RPC-specific facilities but instead, extend what they already provide to work across the wire using HTTP or WebSockets.

On the other side there are tools like [Thrift](https://thrift.apache.org/) or [Apache Arrow RPC](https://arrow.apache.org/docs/format/Flight.html) (among many others) which provide the same features that [Lupine](#1984---implementing-remote-procedure-calls) introduced 40 years ago with the added benefit of performance improvements.

_**rpc-v1.0.1:** All the same but with new names._

## Conclusions

Ok so... we are still using APIs over raw HTTP without much evolving around it. Given that the final destination of most (if not all) of our processes is expected to be a web browser and the amount of effort, time, and money invested in all the machinery around it it's expected that new things need to swim in mud to gain traction. Or they need to have native compatibility with all the web mess, otherwise, they will suffer the "gRPC effect": if you are not 110% compatible with the browser the adoption will be capped at 30% of the market.

As for the initial definition of RPC, it seems that we got it right in the first couple of iterations, at the time of the Cedar project (1984) we had all the features done.
